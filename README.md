...
> [!WARNING]
> 本项目不是投资建议、收益承诺、券商系统或自动交易服务。回测、Paper 状态和
> AI/程序生成的结果都不能证明未来收益。当前实现不连接真实账户、不持有凭证、
> 不自动下单；任何真实数据使用、策略判断和人工交易决定均由使用者自行核验并
> 承担风险。不要向 issue、pull request、日志或仓库提交密钥、真实持仓、账户导出
> 或受限制的行情数据。
