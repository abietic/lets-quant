...
| `execution/alerts.py` | 审计告警生命周期、确认/静默/升级、待投递项和本地回执恢复 | 外部通知送达、修改订单或账户 |
...
- 告警确认只表示人工已看到，不能把 critical 审计问题改成已解决或允许执行。
- 告警静默必须有明确截止时间；到期后恢复提醒，不能永久隐藏未知风险。
- 本地投递回执必须绑定通知载荷哈希；外部通知未接入时不得声称已经送达人。
- 最大回撤只冻结后续调仓，不隐式触发清仓。
