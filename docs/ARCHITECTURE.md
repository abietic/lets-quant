...
| `experiment_replay.py` | 从内嵌合成市场和冻结配置重跑实验，核对输入 ID、结果哈希与摘要 | 重放外部数据源、认证源码身份、证明投资有效性 |
...
- 实验重放必须先通过完整性验证，只接受内嵌确定性合成市场，并要求完整 Python
  版本一致；重跑后输入 ID、结果哈希和摘要必须逐项相同。成功重放仍不证明产物
  来源真实性、源码包身份或投资有效性。
