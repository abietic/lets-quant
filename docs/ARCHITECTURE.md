...
| `engine_inputs.py` | 绑定参考 manifest，加载独立 CSV 或清洗 OHLCV/停牌/企业行动及冻结订单意图 | 丢弃数据 lineage、决定成交或读取参考成交 |
| `vectorbt_adapter.py` | 将订单意图、停牌拒绝和企业行动回调 lowering 后交给 VectorBT 复核组合会计 | 独立生成信号、原生停牌/企业行动账户、盘中撮合 |
| `rqalpha_adapter.py` | 用绑定日线复算 PIT 信号并驱动 RQAlpha 原生事件、撮合、费用及分红/拆并股账户 | 券商执行、复杂公司行动、证明策略有效 |
