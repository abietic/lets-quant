...
| `cross_engine.py` | 候选引擎契约、参考账本绑定、企业行动/文件校验和差异报告 | 生成策略信号、判定投资有效性 |
| `engine_inputs.py` | 绑定参考 manifest，加载初始持仓、独立 CSV 或清洗 OHLCV/停牌/企业行动及冻结订单意图 | 丢弃数据 lineage、决定成交或读取参考成交 |
| `vectorbt_adapter.py` | 将初始持仓、订单意图、停牌和企业行动 lowering 后交给 VectorBT 复核组合会计 | 独立生成信号、原生停牌/企业行动账户、盘中撮合 |
| `rqalpha_adapter.py` | 用绑定日线和原生初始持仓复算 PIT 信号并驱动 RQAlpha 事件、撮合、费用及企业行动账户 | 券商执行、复杂公司行动、证明策略有效 |
